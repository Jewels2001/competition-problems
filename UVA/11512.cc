/*
By: Julie Wojtiw-Quo
Using a suffix array - we can go through the lcp (longest common prefix) array
in order to find the longest substring. Then, we can find the corresponding
suffix and search for the number of occurrences in the suffix array. They will
all be grouped together. We count the number of occurrences/repetitions, and if
either the substring is of length 0 or the number of repetitions is less than 2,
we output "no repetitions found", else we output the substring and the number.
We could use kmp algorithm for O(m) + O(n) time to search for the pattern, but
find function in the suffix array code sufficently provides the answer.
*/
#include <bits/stdc++.h>
using namespace std;
/*
...
int main() {
  ...
  while(t--) {
    string s;
    cin >> s;
    //create suffix array and lcp table
    ...
    string pat = s.substr(sarray[index], ans);
    pair<int,int> a = find(s, sarray, pat);
    int num = a.second - a.first;
    if(ans == 0 || num < 2) {
