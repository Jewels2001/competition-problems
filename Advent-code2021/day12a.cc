...
void dfs(int x, vector<bool> visited, bool twice) {
  //check if small and visited
  if(small[x] && visited[x]) {
    if((getCaveNum("start") == x) || twice) {
      return;
    }
    else if((getCaveNum("end") == x) || twice) {
      ans++;
      return;
    }
    else {
...
int main() {
  ...
  bool twice = false;
  dfs(getCaveNum("start"), visits, twice);
  //output answer
  cout << ans << endl;
